...
def save_metadata_to_json(path, seed_tensor, n_steps=None, n_keypoints=None, fps=None, keypoints=None):
    data = {
...
class GANCardGUI:
    ...
    def setup_image_display(self,
        ...
        progressbar.place(x=75, y=200, width=250, height=20)
        #progressbar.lower()
    ...
    def generate_random_image(self)->None:
        try:
            if self.random_animation_var.get():
                ...
                self.display_image(self.current_image, self.random_image_label, mode='random')
    ...
    def generate_random_gif(self)->Image:
        try:
            ...
            self.current_seed = torch.randn(1, 512, device=device)
            ...
            self.show_progressbar(self.current_n_steps,
                                  "random")
            def worker():
                frames = self.generate_gif_frames(
                    self.current_seed, self.current_n_steps, self.current_n_keypoints,
                    temp_dir, progress_callback=self.update_progressbar)
                self.after_gif_generation(frames, temp_dir)
            threading.Thread(target=worker, daemon=True).start()     
        except Exception as e:
            tkinter.messagebox.showerror("Fehler", f"Fehler beim GIF generieren: {str(e)}")
    

    def after_gif_generation(self, frames, temp_dir):
        greeting_text = self.random_greeting_entry.get()
        position = self.random_position_var.get()
        if greeting_text.strip():
        ...
        frames[0].save(gif_path, save_all=True, append_images=frames[1:], duration=int(1000 / self.current_fps), loop=0)
        im = Image.open(gif_path)
        self.current_image = im.copy()
        im.close()  # GESCHLOSSEN!
        self.animation_frames = frames
        self.is_gif_mode = True
        delay_ms = int(1000 / self.current_fps)
        self.play_gif(frames, self.random_image_label, delay_ms, mode='random')
        shutil.rmtree(temp_dir)
        self.hide_progressbar()


    def after_gif_generation_static(self, frames):
        ...
        self.is_gif_mode = True
        delay_ms = int(1000 / self.current_fps)
        self.play_gif(frames, self.static_image_label, delay_ms, mode='static')
        shutil.rmtree(temp_dir)
        self.hide_progressbar()


    def generate_gif_frames(self, seed, n_steps, n_keypoints, temp_dir, progress_callback=None):
        frames = []

        keypoints = []
        for i in range(n_keypoints):
            keypoints.append(torch.randn(1, 512, device=device))
    ...
    def generate_gif_frames_from_keypoints(self, keypoints, n_steps, temp_dir, progress_callback = None):
        frames = []
    ...
    def generate_static_image(self) -> None:
        try:
            ...
            # ===== 1. JSON ausgewählt? =====
            if file_ext == ".json":
                # Prüfe, ob GIF-JSON (keypoints-Feld vorhanden)
                metadata = {}
                ...
                if metadata.get('keypoints') is not None:
                    ...
                    def worker():
                        frames = self.generate_gif_frames_from_keypoints(
                            self.current_keypoints, self.current_n_steps, tempfile.mkdtemp(),
                    ...
                    def worker():
                        ...
                        self.current_image = hide_seed_tensor_in_image(new_image, self.current_seed)
                        self.is_gif_mode = False
                        self.display_image(self.current_image, self.static_image_label, mode='static')
                        self.hide_progressbar()
                    threading.Thread(target=worker, daemon=True).start()
                else:
                    tkinter.messagebox.showerror("Fehler", "Unbekannter JSON-Inhalt.")

            # ===== 2. GIF? =====
            elif file_ext == ".gif":
                # Suche zugehörige JSON
                json_path = os.path.splitext(file_path)[0] + ".json"
                if not os.path.exists(json_path):
                    tkinter.messagebox.showerror("Fehler", f"Keine passende JSON-Datei gefunden:\n{json_path}")
                    return
            ...
            # ===== 3. Bild? =====
                ...
                tkinter.messagebox.showerror("Fehler", "Nicht unterstütztes Dateiformat.")

        except Exception as e:
            tkinter.messagebox.showerror("Fehler", f"Fehler beim Reproduzieren: {str(e)}")


    def show_progressbar(self, max_steps,
                         mode = "random"
                         ):
        if mode == "random":
    ...
    def play_gif(self, frames, label, delay_ms=100, index=0, mode='random'):
        # Der richtige after_id für das jeweilige Tab
        after_id_attr = f'{mode}_gif_after_id'

        # Stoppe vorhergehende Animation beim Frame 0 (also bei neuen GIF)
        if index == 0:
            prev_id = getattr(self, after_id_attr, None)
            if prev_id:
        ...
        # Speichere die after_id für das richtige Tab
        after_id = self.window.after(delay_ms, self.play_gif, frames, label, delay_ms, (index + 1) % len(frames), mode)
        setattr(self, after_id_attr, after_id)


    def pad_center_image(self, img, target_size=(300, 350), bg_color=(33, 48, 85)):
        w, h = img.size
        tw, th = target_size
    ...
    def display_image(self, image, target_label, mode='random'):
        # Vorherige Animation im entsprechenden Tab abbrechen
        after_id_attr = f'{mode}_gif_after_id'
        prev_id = getattr(self, after_id_attr, None)
    ...
    def save_image(self):
        ...
        try:
            if self.is_gif_mode:
                ...
                # Statisches Bild speichern inkl. Seed-JSON im Ordner
                file_path = filedialog.asksaveasfilename(
                ...
                if file_path:
                    ...
                    # JSON speichern – nur seed!
                    json_path = os.path.join(save_dir, f"{base_name}.json")
                    save_metadata_to_json(json_path, seed_tensor=self.current_seed)
                    tkinter.messagebox.showinfo("Erfolg", "Bild und Seed als JSON erfolgreich gespeichert!")           
        except Exception as e:
