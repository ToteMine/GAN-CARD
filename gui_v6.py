...
class GANCardGUI:
    ...
    def generate_static_image(self) -> None:
        try:
            ...
            if file_ext == ".json":
                # JSON direkt laden
                self.current_seed, self.current_n_steps, self.current_n_keypoints, self.current_fps, self.current_keypoints = load_metadata_from_json(file_path)

                self.show_progressbar(self.current_n_steps)
                def worker():
                    frames = self.generate_gif_frames_from_keypoints(
                        self.current_keypoints, self.current_n_steps, tempfile.mkdtemp(),  # wir passen generate_gif_frames_from_keypoints gleich an!
                        progress_callback=self.update_progressbar)
                    self.after_gif_generation_static(frames)
                threading.Thread(target=worker, daemon=True).start()

            elif file_ext == ".gif":
                # Zugehörige JSON zur GIF suchen
                ...
                self.current_seed, self.current_n_steps, self.current_n_keypoints, self.current_fps, self.current_keypoints = load_metadata_from_json(json_path)
                ...
                # Normales Bild mit verstecktem Seed
    ...
    def show_progressbar(self, max_steps):
        ...
        self.progress_win.title("Generiere GIF ...")
        self.progressbar = ttk.Progressbar(self.progress_win, orient="horizontal",
                                           length=300, mode="determinate", maximum=max_steps)
